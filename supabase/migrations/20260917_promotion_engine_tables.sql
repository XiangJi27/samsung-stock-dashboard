...
-- ============================================================================
-- 11b. REJECT PROMOTION CAMPAIGN RPC (Atomic Single Transaction Rejection)
-- Rejects a draft/review/approved campaign atomically.
-- Active campaigns must be rolled back or superseded instead.
-- ============================================================================
...
begin
  ...
  if v_campaign.status = 'ACTIVE' then
    raise exception using
      ...
      detail = 'Use rollback_promotion_campaign or activate a replacement campaign.';
  end if;
  ...
  if not (v_campaign.status = any(v_allowed_statuses)) then
    raise exception using
      errcode = 'P0001',
      message = 'CAMPAIGN_STATUS_NOT_REJECTABLE',
      detail = format('Current campaign status is %s', v_campaign.status);
  end if;
  ...
  select count(*)::integer into v_open_error_count
  ...
  -- 10. Keep validation errors for audit (Audit Log records rejection)
  ...
  values (
    ...
    jsonb_build_object(
      ...
      'approvedBy', v_campaign.approved_by,
      'approvedAt', v_campaign.approved_at
    ),
    jsonb_build_object(
      'campaignStatus', 'REJECTED',
      'batchStatus', 'REJECTED',
      'offerCount', v_offer_count,
      'openValidationErrorCount', v_open_error_count
    ),
    p_user_id,
    now(),
    trim(p_reason)
  ...
  return jsonb_build_object(
    'status', 'REJECTED',
    'campaignId', p_campaign_id,
    'importBatchId', v_campaign.import_batch_id,
    'branchCode', v_campaign.branch_code,
    'previousCampaignStatus', v_campaign.status,
    'previousBatchStatus', v_batch.status,
    'offerCount', v_offer_count,
    'openValidationErrorCount', v_open_error_count,
    'rejectedBy', p_user_id,
...
revoke execute on function public.reject_promotion_campaign(uuid, uuid, text, text) from authenticated, anon, public;
revoke execute on function public.activate_promotion_campaign(uuid, uuid, uuid) from authenticated, anon, public;
revoke execute on function public.rollback_promotion_campaign(uuid, uuid, uuid, text) from authenticated, anon, public;

grant execute on function public.create_promotion_draft_batch(jsonb, uuid) to postgres, service_role;
grant execute on function public.reject_promotion_campaign(uuid, uuid, text, text) to postgres, service_role;
