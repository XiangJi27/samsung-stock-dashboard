...
with open("promotion_draft.csv", "w", encoding="utf-8-sig", newline="") as f:
    ...
    for v in existing_variants:
        row = {
            ...
            "rrp": v.get("rrp", ""),
            "discount": v.get("discount") if (v.get("discount") is not None and v.get("discount") != "") else (v.get("discountValue") if v.get("discountValue") is not None else v.get("addOnDiscount", "")),
